...
class XmlSignatureWrappingTest extends BaseTestCase
{
    ...
    public function test_rejects_duplicate_id_even_when_signed_element_is_untouched(): void
    {
        // The genuine signed assertion is left byte-for-byte intact so its digest and signature
        // still verify. A decoy element carrying the same ID is appended after it. Without the
        // guard, validate() returns true for a document that contains duplicate IDs — the
        // structural precondition for XSW. The guard must reject it.
        $assertionId = Helper::generateID();
        $response = $this->buildSignedResponse($assertionId, 'alice@example.com');
